...
//         RNToasty.Show({ title: 'Login Successfully', position: 'center' });
//       } else {
//         RNToasty.Show({ title: 'Login Failed: Invalid credentials', position: 'center' });
//       }
//     } catch (error) {
//       RNToasty.Error({ title: error?.response?.data?.error || 'Login failed' });
//     } finally {
...
const Login = ({ navigation, setToken }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      // If credentials are correct, proceed with API request
      const response = await axios.post('https://reqres.in/api/login', { email, password });
      const token = response?.data?.token;
